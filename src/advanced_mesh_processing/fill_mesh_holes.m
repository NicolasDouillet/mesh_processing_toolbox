...
%% fill_mesh_holes : function to fill the mesh (T) holes.
...
holes_begin_id = find(cellsz <= max_perim_sz);
...
for h = holes_begin_id:nb_holes % loop on every holes
    ...
    if bound_nb_vtx <= max_perim_sz % current hole perimeter (bound_nb_vtx) smaller than the max defined                                                                              
        ...
        while bound_nb_vtx > 2 % smallest hole is a triangle (3 edges)                                                                                       
            ...
            new_triangle = [mat_boundary_forward(min_angle_id), mat_boundary(min_angle_id), mat_boundary_backward(min_angle_id)];                                                
