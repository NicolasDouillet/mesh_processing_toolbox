...
%          [  |     |     |  ]
%
% - nb_it : positive integer scalar double, the number of iterations, the "depth" erosion level. Optional.
%
...
for k = 1:nb_it
    ...
    raw_edg_list = query_edg_list(T_out);
    [~,~,iu] = unique(sort(raw_edg_list,2),'rows');
    
    % Unique edges only, even in mirror
    [i3,~,i4] = histcounts(iu,unique(iu));
    lone_edges_id_vect = i3(i4) == 1;
    lone_edg_list = raw_edg_list(lone_edges_id_vect,:);
    
    if strcmpi(mode,'smooth')
        
        bound_vertices = unique(lone_edg_list(:))';
        [V_out,T_out] = remove_vertices(bound_vertices,V_out,T_out,'index');
        
    elseif strcmpi(mode,'saw')
        
        tgl_id_list = cell2mat(find_triangle_indices_from_edg_list(T_out,lone_edg_list));
