function [b, nmnfld_edg_idx] = ismesh2Dmanifold(V, T)
...
%
% - nmnfld_edg_idx : positive integer column vector double, the index list of
...
for n = 1:size(tgl_idx_list,1)
    
    if numel(cell2mat(tgl_idx_list(n,1))) > 2 % non manifold edge
        
        nmnfld_edg_nb = nmnfld_edg_nb + 1;
        nmnfld_edg_idx = cat(2,nmnfld_edg_idx,n);
        b = false;
...
[V_test,~] = remove_vertices(nmnfld_vtx_idx,V,T,'indices');
